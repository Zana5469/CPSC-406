...
\subsubsection{Question}
Can we use resolution or Horn resolution in any aspects of turing machines?
...
\textbf{Problem:} Show that the solution of $T(n) = T(n-1) + n$ is $O(n^2)$.

\textbf{Solution:}
We use the substitution method. We want to show that $T(n) \leq cn^2$ for some constant $c > 0$ and $n \geq n_0$.
\begin{itemize}
    \item \textbf{Inductive Hypothesis:} Assume $T(k) \leq ck^2$ for all $k < n$.
    ...
    &= cn^2 - (2cn - c - n)
    \end{align*}
    To satisfy $T(n) \leq cn^2$, we need $2cn - c - n \geq 0$.
    \begin{align*}
    n(2c - 1) &\geq c \implies n \geq \frac{c}{2c - 1}
    \end{align*}
    For $c=1$, this holds for $n \geq 1$. Thus, $T(n) = O(n^2)$.
\end{itemize}

\textbf{Exercise 4.3-2}
\textbf{Problem:} Show that the solution of $T(n) = T(\lceil n/2 \rceil) + 1$ is $O(\lg n)$.

\textbf{Solution:}
We want to show $T(n) \leq c \lg(n - b)$ for constants $c, b$.
\begin{itemize}
    \item \textbf{Inductive Hypothesis:} Assume $T(k) \leq c \lg(k - b)$ for $k < n$.
    \item \textbf{Inductive Step:}
    \begin{align*}
    T(n) &\leq c \lg(\lceil n/2 \rceil - b) + 1 \\
    &\leq c \lg\left(\frac{n+1}{2} - b\right) + 1 \\
    &= c \lg\left(\frac{n + 1 - 2b}{2}\right) + 1 \\
    &= c \lg(n + 1 - 2b) - c \lg 2 + 1 \\
    &= c \lg(n + 1 - 2b) - c + 1
    \end{align*}
    We need $c \lg(n + 1 - 2b) - c + 1 \leq c \lg(n - b)$. 
    Choosing $b=1$, we get $c \lg(n - 1) - c + 1 \leq c \lg(n - 1)$, which holds if $-c + 1 \leq 0$, i.e., $c \geq 1$.
\end{itemize}

\textbf{Exercise 4.3-3}
\textbf{Problem:} Show that $T(n) = 2T(\lfloor n/2 \rfloor) + n$ is $\Omega(n \lg n)$ and conclude $T(n) = \Theta(n \lg n)$.

\textbf{Solution:}
We want to show $T(n) \geq cn \lg n$.
\begin{itemize}
    \item \textbf{Inductive Hypothesis:} Assume $T(k) \geq ck \lg k$ for $k < n$.
    \item \textbf{Inductive Step:}
    \begin{align*}
    T(n) &= 2T(\lfloor n/2 \rfloor) + n \\
    &\geq 2c \lfloor n/2 \rfloor \lg \lfloor n/2 \rfloor + n \\
    &\geq 2c \left(\frac{n-1}{2}\right) \lg \left(\frac{n-1}{2}\right) + n \\
    &= c(n-1)(\lg(n-1) - 1) + n \\
    &= c(n-1)\lg(n-1) - c(n-1) + n
    \end{align*}
    Since $T(n) = O(n \lg n)$ is known and we have shown $T(n) = \Omega(n \lg n)$, then $T(n) = \Theta(n \lg n)$.
\end{itemize}

\subsubsection{Question}
How does proving recurrences help us apply concepts related to Big O to to programming?

\subsection{Week 11}

\subsubsection{HW 11}

\textbf{Exercise 4.4-2: $T(n) = T(n/2) + n^2$} \\
...
\end{tikzpicture}
\end{center}
\textit{Note: Since $a=1$, the tree is a single linear chain of subproblems.}

\vspace{2em}

\textbf{Exercise 4.4-5: $T(n) = T(n-1) + T(n/2) + n$} \\
\textbf{Recursion Tree Analysis:}
\begin{center}
...
\end{tikzpicture}
\end{center}
\textit{Note: This tree is highly unbalanced. The left branches ($n-1$) reach a depth of $n$, while the right branches ($n/2$) reach a depth of $\lg n$.}

\textbf{Exercise 4.5-1: Master Theorem Applications}

\begin{enumerate}[label=\alph*.]
    \item \textbf{$T(n) = 2T(n/4) + 1$} \\
    $f(n) = 1$. Since $1 = O(n^{1/2 - \epsilon})$ for $\epsilon = 0.5$, this fits \textbf{Case 1}. \\
    \textit{Result:} $T(n) = \Theta(\sqrt{n})$.

    \item \textbf{$T(n) = 2T(n/4) + \sqrt{n}$} \\
    $f(n) = \sqrt{n}$. Since $\sqrt{n} = \Theta(n^{1/2})$, this fits \textbf{Case 2}. \\
    \textit{Result:} $T(n) = \Theta(\sqrt{n} \lg n)$.

    \item \textbf{$T(n) = 2T(n/4) + n$} \\
    $f(n) = n$. Since $n = \Omega(n^{1/2 + \epsilon})$ for $\epsilon = 0.5$, and the regularity condition $2(n/4) \leq cn$ holds for $c=1/2$, this fits \textbf{Case 3}. \\
    \textit{Result:} $T(n) = \Theta(n)$.

    \item \textbf{$T(n) = 2T(n/4) + n^2$} \\
    $f(n) = n^2$. Since $n^2 = \Omega(n^{1/2 + \epsilon})$ for $\epsilon = 1.5$, and the regularity condition $2(n/4)^2 \leq cn^2$ holds for $c=1/8$, this fits \textbf{Case 3}. \\
    \textit{Result:} $T(n) = \Theta(n^2)$.
\end{enumerate}

\textbf{Extra Credit:}
\textbf{Exercise 4.5-4: $T(n) = 4T(n/2) + n^2 \lg n$}
\textbf{Analysis of Applicability:}
To use the Master Method, we identify $a = 4$, $b = 2$, and $f(n) = n^2 \lg n$.
The watershed function is $n^{\log_b a} = n^{\log_2 4} = n^2$.

\begin{itemize}
    \item We compare $f(n) = n^2 \lg n$ with $n^{\log_b a} = n^2$.
    \item While $f(n)$ is asymptotically larger than $n^2$, it is not \textit{polynomially} larger. For any $\epsilon > 0$, $f(n) \neq \Omega(n^{2+\epsilon})$ because the $\lg n$ factor grows slower than any polynomial $n^\epsilon$.
    \item Consequently, the recurrence falls into the "gap" between Case 2 and Case 3 of the standard Master Theorem.
\end{itemize}

\textbf{Conclusion on Applicability:} 
No, the standard Master Method cannot be applied because $f(n)$ is not polynomially larger than $n^{\log_b a}$.

\vspace{1em}

\textbf{Asymptotic Upper Bound:}
Although the standard version fails, we can use the \textbf{Extended Master Theorem} (Case 2), which states that if $f(n) = \Theta(n^{\log_b a} \lg^k n)$ for $k \geq 0$, then $T(n) = \Theta(n^{\log_b a} \lg^{k+1} n)$.
\begin{itemize}
    \item Here, $n^{\log_b a} = n^2$ and $f(n) = n^2 \lg^1 n$, so $k = 1$.
    \item $T(n) = \Theta(n^2 \lg^{1+1} n) = \Theta(n^2 \lg^2 n)$.
\end{itemize}

\textbf{Result:} $T(n) = O(n^2 \lg^2 n)$.

\subsubsection{Question}
How can the Master Theorem help us with Turing Machines, if it's possible?
...
To find the expected value of the sum of $n$ standard six-sided dice, we define indicator random variables for each die.

\begin{enumerate}
    \item \textbf{Define Variables:} Let $X_i$ be the random variable representing the value of the $i$-th die for $i = 1, 2, \dots, n$.
    \item \textbf{Expected Value of one die:} Since each face $\{1, 2, 3, 4, 5, 6\}$ has an equal probability of $1/6$, the expected value $E[X_i]$ is:
    \[
    E[X_i] = \sum_{j=1}^{6} j \cdot P(X_i = j) = \frac{1+2+3+4+5+6}{6} = \frac{21}{6} = 3.5
    \]
    \item \textbf{Total Expectation:} Let $X$ be the total sum, where $X = \sum_{i=1}^{n} X_i$. By the linearity of expectation:
    \[
    E[X] = E\left[\sum_{i=1}^{n} X_i\right] = \sum_{i=1}^{n} E[X_i]
    \]
    \[
    E[X] = \sum_{i=1}^{n} 3.5 = 3.5n
    \]
\end{enumerate}

\textbf{Result:} The expected sum of $n$ dice is $3.5n$.

\textbf{Exercise 5.2-4}

We want to find the expected number of customers who receive their own hat back when $n$ hats are returned in a random order.

\begin{enumerate}
    \item \textbf{Define Indicator Variables:} Let $X_i$ be an indicator random variable for customer $i$, where:
    \[
    X_i = I\{\text{customer } i \text{ gets their own hat back}\} = 
    \begin{cases} 
    1 & \text{if customer } i \text{ gets their own hat} \\
    0 & \text{otherwise}
    \end{cases}
    \]
    \item \textbf{Probability of Success:} Because the hats are returned in a random permutation, each customer has a $1/n$ chance of receiving their specific hat:
    \[
    P(X_i = 1) = \frac{1}{n}
    \]
    \item \textbf{Expected Value of $X_i$:} For an indicator variable, the expectation is equal to the probability of the event occurring:
    \[
    E[X_i] = 1 \cdot P(X_i = 1) + 0 \cdot P(X_i = 0) = \frac{1}{n}
    \]
    \item \textbf{Total Expectation:} Let $X$ be the total number of customers who receive their own hat, where $X = \sum_{i=1}^{n} X_i$. By linearity of expectation:
    \[
    E[X] = \sum_{i=1}^{n} E[X_i] = \sum_{i=1}^{n} \frac{1}{n} = n \cdot \left(\frac{1}{n}\right) = 1
    \]
\end{enumerate}

\textbf{Result:} The expected number of customers who get back their own hat is $1$.

\subsubsection{Question}
Why are indicator random variables important/necessary? What is their purpose?

\subsection{Week 13}

\subsubsection{HW 13}

\textbf{Sudoku Encoding}

Let $x_{r,c,v}$ be a boolean variable that is true if and only if the entry at row $r$ and column $c$ has value $v$. The following conditions define a valid Sudoku:

\begin{itemize}
    \item \textbf{$C_1$: Each entry has at least one value}
    \[ \bigwedge_{r=1}^9 \bigwedge_{c=1}^9 \left( \bigvee_{v=1}^9 x_{r,c,v} \right) \]

    \item \textbf{$C_2$: Each entry has at most one value}
    \[ \bigwedge_{r=1}^9 \bigwedge_{c=1}^9 \bigwedge_{v_1=1}^8 \bigwedge_{v_2=v_1+1}^9 (\neg x_{r,c,v_1} \vee \neg x_{r,c,v_2}) \]

    \item \textbf{$C_3$: Each row has all numbers}
    \[ \bigwedge_{r=1}^9 \bigwedge_{v=1}^9 \left( \bigvee_{c=1}^9 x_{r,c,v} \right) \]

    \item \textbf{$C_4$: Each column has all numbers}
    \[ \bigwedge_{c=1}^9 \bigwedge_{v=1}^9 \left( \bigvee_{r=1}^9 x_{r,c,v} \right) \]

    \item \textbf{$C_5$: Each $(3 \times 3)$-block has all numbers}
    \[ \bigwedge_{I=0}^2 \bigwedge_{J=0}^2 \bigwedge_{v=1}^9 \left( \bigvee_{r=3I+1}^{3I+3} \bigvee_{c=3J+1}^{3J+3} x_{r,c,v} \right) \]

    \item \textbf{$C_6$: Respect given clues}
    \[ \bigwedge_{(r,c,v) \in \text{Clues}} x_{r,c,v} \]
\end{itemize}

\textbf{Probabilistic Algorithm Analysis}

\textbf{1. Worst-case Running Time}
The total number of inner loop iterations is determined by the summation of the range of $j$ over the range of $i$:
\[ \sum_{i=1}^{n} 2i = 2 \sum_{i=1}^{n} i = 2 \cdot \frac{n(n+1)}{2} = n^2 + n \]
The worst-case running time is therefore \textbf{$O(n^2)$}.

\textbf{2. Expected Value $\mathbb{E}[X]$}
Let $X_{i,j}$ be an indicator random variable for the $j$-th roll in the $i$-th iteration. Since the probability of rolling $r \ge 5$ is $P = \frac{2}{6} = \frac{1}{3}$:
\[ \mathbb{E}[X] = \sum_{i=1}^n \sum_{j=1}^{2i} \mathbb{E}[X_{i,j}] = \sum_{i=1}^n \sum_{j=1}^{2i} \frac{1}{3} \]
\[ \mathbb{E}[X] = \sum_{i=1}^n \frac{2i}{3} = \frac{2}{3} \cdot \frac{n(n+1)}{2} = \frac{n(n+1)}{3} \]

\textbf{3. Correction of Student Claim}
The student's claim that $\mathbb{E}[X] = n \cdot \frac{2}{6}$ is incorrect because it ignores the inner loop. The algorithm performs $n^2+n$ rolls in total, not $n$.

\textbf{4. Variation for $r = 6$}
If the condition changes to $r = 6$, the probability $P$ becomes $\frac{1}{6}$:
\[ \mathbb{E}[X] = \sum_{i=1}^n \frac{2i}{6} = \frac{1}{3} \cdot \frac{n(n+1)}{2} = \frac{n(n+1)}{6} \]

\subsubsection{Question}
Where is probabilistic algorithm analysis the most beneficial and why is Big O important here?
